...
tweets_df_senti  %>%
  ...
  summarise(text_p = paste(text, collapse = " ")) %>%
  mutate(Sentiment_syuzhet = syuzhet::get_sentiment(text_p, method = "syuzhet", language = "spanish")) %>%
...
dtm <- convert(mydfm_fa, to = "topicmodels")
